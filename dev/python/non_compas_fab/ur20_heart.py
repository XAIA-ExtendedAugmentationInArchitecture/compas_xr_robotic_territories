...
# urdf_path = "/home/raj/Projects/compas_xr_robotic_territories/dev/scripts/urdf/ur_description/urdf/ur20.urdf"
urdf_path = r"C:\Users\jk6372\Desktop\00_princeton_projects\00_robotic_territories\00_git\compas_xr_robotic_territories\dev\scripts\urdf\ur_description\urdf\ur20.urdf"
TIP_IDX = 8

def get_link_state(robot_id, link_id):
    tip_link_state = p.getLinkState(robot_id, link_id)
    tip_position = tip_link_state[0]  # Cartesian position (x, y, z)
    tip_orientation = tip_link_state[1]  # Quaternion orientation (x, y, z, w)

    return np.array(tip_position), np.array(tip_orientation)
...
if __name__ == "__main__":
    ...
    robot_id, joints = load_robot(urdf_path)
    print(joints)

    tip_link_index = 8

    # # Get the position and orientation of the tip (tool attachment point)
    # tip_link_state = p.getLinkState(robot_id, tip_link_index)
    # tip_position = tip_link_state[0]  # Cartesian position (x, y, z)
    # tip_orientation = tip_link_state[1]  # Quaternion orientation (x, y, z, w)

    frames = json_load(r"C:\Users\jk6372\Desktop\00_princeton_projects\00_robotic_territories\00_git\compas_xr_robotic_territories\dev\python\frames.json")
    points = []
    target_point_idx = []
    quaternions = []

    for i,frame in enumerate(frames):
        point_data = frame.point
        quaternion = Quaternion.from_frame(frame)
        quaternions.append(quaternion)
        create_marker_point(point_data[0], point_data[2], point_data[1], [0.025, 0.025, 0.025])
        points.append(point_data)
        target_point_idx.append(i)

    
    # with PyBulletClient(connection_type='direct') as client:
    #     mesh = json_load(r"C:\Users\jk6372\Desktop\00_princeton_projects\00_robotic_territories\00_git\compas_xr_robotic_territories\dev\fabrication\rhino\fabrication\setup\table\table_test.json")
    #     print (type(mesh))
    #     cm = CollisionMesh(mesh, 'table')
    #     print (type(cm))
    ...
    while curr_point_idx < len(target_point_idx):
        if (
            euclidean_distance(
                ...
                get_link_state(robot_id, TIP_IDX)[0] , points[target_point_idx[curr_point_idx]]
            )
            < 0.1
        ):
            create_marker_point(
        ...
        joint_poses = p.calculateInverseKinematics(
            robot_id,
            TIP_IDX,  # 'gripper_finger_joint'
            points[target_point_idx[curr_point_idx]],
            # p.getQuaternionFromEuler([0, 0, 0]),
            quaternions[target_point_idx[curr_point_idx]],

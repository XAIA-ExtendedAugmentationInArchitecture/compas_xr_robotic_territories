...
class UR20Env:
    ...
    def reset(self) -> np.ndarray:
        ...
        return np.concatenate([initial_pos, initial_ori]), self.episode_target, {"current_oreintation": initial_ori}
    ...
    def step(self, action: np.ndarray) -> Tuple[np.ndarray, float, bool, Dict[str, Any]]:
        ...
        return np.concatenate([current_pos, current_ori]), int(success), termination, truncation, info
    ...
    def _random_target(self) -> np.ndarray:
        ...
        if self.render_env:
            ...
            shape_id = p.createVisualShape(p.GEOM_BOX, halfExtents=[0.05]*3, rgbaColor=[0, 255, 0, 255],)

...
URDF_PATH = "urdf/ur_20_attached_ee/urdf/ur20_ee.urdf"
...
class UR20PickandPlaceEnv:
    def __init__(self, reward_type: int = 1, render: bool = True):
        ...
        self.default_position = [1.7, 0.0, 1.0]
        self.default_orientation = [np.pi, 0, 0]

        self._box_sample_min = np.array([0.7, -0.5, 0.15])
        self._box_sample_max = np.array([1.5, 0.5, 0.15])

        self._sample_min = np.array([0.7, -0.5, 0.15])

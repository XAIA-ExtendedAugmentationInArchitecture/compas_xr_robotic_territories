...
OUTPUT_PATH = r"C:\Users\jk6372\Desktop\00_princeton_projects\00_robotic_territories\00_git\compas_xr_robotic_territories\dev\nat_net_lib_test\samples\random_test_mocap.json"

rigid_body_names = {}

# def receive_rigid_body_frame_TEST(new_id, position, rotation):
#     global last_print_time, output_data, last_write_time
...
#     # Optional: throttle terminal printing
#     if current_time - last_print_time > 1.0:
#         print(f"[{time.strftime('%H:%M:%S')}] Rigid Body {new_id} - Pos: {position}, Rot: {rotation}")
#         last_print_time = current_time

#     # Optional: write to file every WRITE_INTERVAL seconds
...
def receive_rigid_body_frame_TEST(new_id, position, rotation):
    global last_print_time, output_data, last_write_time, rigid_body_names

    current_time = time.time()

    model_name = rigid_body_names.get(new_id, "Unknown")
    ...
    # print ("I AM GETTING CALLED FOR RIGID BODY", new_id, ":", model_name, "at", time.strftime('%H:%M:%S'), "with position", position, "and rotation", rotation)
    print(f"Frame Info: {frame_info['timestamp']} - Rigid Body {model_name} (ID {new_id}) - Pos: {frame_info['position']}, Rot: {frame_info['rotation']}")
    output_data.append(frame_info)

    # if current_time - last_print_time > 1.0:
    #     print(f"[{time.strftime('%H:%M:%S')}] {model_name} (ID {new_id}) - Pos {frame_info['position']}, Rot {frame_info['rotation']}")
    #     last_print_time = current_time
...
def get_rigid_body_names(client):
    global rigid_body_names
    descriptions = client.get_data_descriptions()
    if descriptions is None:
        print("No data descriptions received.")
        return

    for desc in descriptions:
        if desc.type == 0:  # 0 = Rigid Body
            rigid_body_names[desc.id] = desc.name
            print(f"Registered Rigid Body ID {desc.id} with name '{desc.name}'")
...
#     # optionsDict["clientAddress"] = "169.254.24.199"
#     # optionsDict["serverAddress"] = "169.254.24.199"
...
#     # get_rigid_body_names(streaming_client)
    
#     streaming_client.set_client_address(optionsDict["clientAddress"])
#     streaming_client.set_server_address(optionsDict["serverAddress"])
#     streaming_client.set_use_multicast(optionsDict["use_multicast"])
#     streaming_client.set_print_level(0) #TODO: Changed this here.

#     # Configure the streaming client to call our rigid body handler on the emulator to send data out.
#     # streaming_client.new_frame_listener = receive_new_frame
#     # streaming_client.rigid_body_listener = receive_rigid_body_frame
#     streaming_client.new_frame_listener = None  #TODO: TESTING.....
#     streaming_client.rigid_body_listener = receive_rigid_body_frame_TEST  #TODO: TESTING.....
...
#     print("Streaming Thread Started: %s" % is_running)
#     if not is_running:
...
#                 print("\nInterrupt received. Breaking loop.")
#                 break
...
OUTPUT_PATH = r"C:\Users\jk6372\Desktop\00_princeton_projects\00_robotic_territories\00_git\compas_xr_robotic_territories\dev\nat_net_lib_test\samples\rigid_body_again_test.json"
# OUTPUT_PATH = "rigid_body_output.json"

output_data = []  # Global to store data from the listener
...
if __name__ == "__main__":
    ...
    streaming_client.set_server_address(optionsDict["serverAddress"])
    streaming_client.set_use_multicast(optionsDict["use_multicast"])
    # streaming_client.set_print_level(0)  # Minimized output
    streaming_client.new_frame_listener = None
    streaming_client.rigid_body_listener = receive_rigid_body_frame_TEST  # Your custom function
    ...
    print("Collecting data for 12 seconds...")
    try:
        time.sleep(5)  # Replace the user input loop with this timer
    except KeyboardInterrupt:
        print("Keyboard interrupt received. Shutting down early.")

    print("Finished. Shutting down streaming client...")
    streaming_client.shutdown()
